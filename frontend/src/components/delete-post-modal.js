...
export default function DeletePostModal({ buttonRef, postID, closeDeleteModal, onPostDeleted }) {
    ...
    useEffect(() => {
        if (buttonRef.current) {
            ...
            setModalStyle({
                position: "absolute",
                top: rect.top + scrollTop + 70, // Adjust height above the button
                left: rect.left + 30,
    ...
    const { mutateAsync, isLoading, error } = useMutation({
        mutationFn: async (requestData) => {
            const response = await api.delete('/delete-post/', { data: requestData });
        ...
        onSuccess: (data) => {
            ...
            closeDeleteModal();
            onPostDeleted() // Notify Parent to invalidate query
        },
        onError: (error) => {
